#!/usr/bin/env python3
"""
TEST version of mint_strategy_returns — points at local Supabase mock.
Run AFTER starting scripts/test_db/supabase_mock.py in another terminal.

Expected result with test data (prices in cents):
  BHG.JO latest price: 71400 cents → R714
  VOD.JO latest price: 16000 cents → R160
  Strategy holds 1× BHG.JO + 2× VOD.JO
  basket_value on 2025-01-08 should be R1034  (1×714 + 2×160)
  NOT R103400 (which is what the old cent-bug produced)
"""
...
def process_strategy(strategy_id, holdings):
    ...
    for as_of_date in common_dates:
        ...
        for h in holdings:
            ...
            current_price_rands = float(current_price) / 100
            ...
            for p in PERIODS:
                ...
                if abs_val is not None:
                    ...
                    period_pnls[p] += shares * (float(abs_val) / 100)
                else:
    ...
    # Verification
    print(f"\n  --- VERIFICATION ---")
    print(f"  Expected basket_value on 2025-01-08: R1034.00")
    print(f"    (1 share BHG.JO @ R714 + 2 shares VOD.JO @ R160)")
    last = rows_to_insert[-1]
    print(f"  Computed basket_value on {last['as_of_date']}: R{last['basket_value']:.2f}")
    print(f"  {'✓ PASS' if abs(last['basket_value'] - 1034.0) < 0.01 else '✗ FAIL — check cent conversion'}")


def main():
    print("\nTest run — strategy returns (local DB)")
    strategies = supabase_request("GET", "strategies_c?select=id,holdings")
    if not strategies:
        print("No strategies found. Is the mock server running? Is data seeded?")
        return
