#!/usr/bin/env python3
"""
TEST version of mint_client_returns — points at local Supabase mock.
Run AFTER starting scripts/test_db/supabase_mock.py in another terminal.

Expected result with test data:
  User holds 1 share BHG.JO, avg_fill = 70000 cents (R700)
  BHG.JO price on 2025-01-08 = 71400 cents → R714
  Expected inception PnL: 1 × (714 - 700) = R14 (NOT R1400)
  Expected basket_value on 2025-01-08: R714
"""
...
SUPABASE_URL = "http://localhost:5001"
...
DEBUG_USER = "dddddddd-0000-0000-0000-000000000001"
...
def process_client_strategy(user_id, strategy_id, holdings, debug=False):
    ...
    for as_of_date in all_dates:
        ...
        # inception_cost = cost of current open positions only
        ...
        result = {
            ...
            "inception_pnl"    : round(inception_pnl, 4),
            "inception_pct"    : round(inception_pct, 6) if inception_pct is not None else None,
        ...
        for p in PERIOD_LOOKBACK:
            ...
            result[f"{p}_pnl"] = round(pnl, 4)
            result[f"{p}_pct"] = round(pct, 6) if pct is not None else None
    ...
    print(f"    Date: {last['as_of_date']}")
    print(f"    basket_value  expected: R714.00   got: R{last['basket_value']:.2f}  {'✓' if abs(last['basket_value'] - 714.0) < 0.01 else '✗'}")
    print(f"    inception_pnl expected: R14.00    got: R{last['inception_pnl']:.2f}  {'✓' if abs(last['inception_pnl'] - 14.0) < 0.01 else '✗'}")
    print(f"    inception_pct expected: ~2.00%    got: {last['inception_pct']:.4f}%")


def main():
    print("\nTest run — client returns (local DB)")
    holdings = fetch_all_holdings()
    if not holdings:
        print("No holdings found. Is the mock server running? Is data seeded?")
        return
