...
// ============ BANKING API ENDPOINTS (delegated to ESM handlers) ============

app.post("/api/banking/initiate", async (req, res) => {
  const { default: handler } = await import('../api/banking/initiate.js');
  return handler(req, res);
});

app.get("/api/banking/status", async (req, res) => {
  const { default: handler } = await import('../api/banking/status.js');
  return handler(req, res);
});

app.post("/api/banking/capture", async (req, res) => {
  const { default: handler } = await import('../api/banking/capture.js');
  return handler(req, res);
});
...
app.post('/api/credit-check', async (req, res) => {
  const { default: handler } = await import('../api/credit-check.js');
  return handler(req, res);
});

app.get('/api/mock-mode', async (req, res) => {
  const { default: handler } = await import('../api/mock-mode.js');
  return handler(req, res);
});
