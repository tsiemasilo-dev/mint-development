...
// Brand-specific watermark for cards
    ...
    <div className="min-h-screen pb-32 relative overflow-x-hidden text-slate-900 bg-slate-50">
      ...
      <div className="absolute inset-x-0 top-0 -z-10 h-full">
        <div 
          ...
          style={{ 
            height: '100vh',
            background: 'linear-gradient(180deg, #0d0d12 0%, #0e0a14 0.5%, #100b18 1%, #120c1c 1.5%, #150e22 2%, #181028 2.5%, #1c122f 3%, #201436 3.5%, #25173e 4%, #2a1a46 5%, #301d4f 6%, #362158 7%, #3d2561 8%, #44296b 9%, #4c2e75 10%, #54337f 11%, #5d3889 12%, #663e93 13%, #70449d 14%, #7a4aa7 15%, #8451b0 16%, #8e58b9 17%, #9860c1 18%, #a268c8 19%, #ac71ce 20%, #b57ad3 21%, #be84d8 22%, #c68edc 23%, #cd98e0 24%, #d4a2e3 25%, #daace6 26%, #dfb6e9 27%, #e4c0eb 28%, #e8c9ed 29%, #ecd2ef 30%, #efdaf1 31%, #f2e1f3 32%, #f4e7f5 33%, #f4e7f5 100%)'
          }} 
        />
      </div>
      ...
      <div className="px-5 pt-12 pb-8">
        ...
        <div className="grid grid-cols-4 gap-3 mb-10 text-[11px] font-medium">
          {[
            ...
            return (
              <button 
                key={i} 
                className="flex flex-col items-center gap-2 rounded-2xl bg-white px-2 py-3 text-slate-700 shadow-md transition-all active:scale-95 active:shadow-sm border border-slate-100/50"
              >
        ...
        {/* Asset List - RESTYLED to Mint Cards */}
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                   ...
                   <div className="flex items-center gap-3">
                      <div className="text-right">
                        <span className="inline-block rounded-full bg-slate-100 px-2.5 py-1 text-[9px] font-black text-slate-600 uppercase">
                          LTV {item.ltv}
                        </span>
                      </div>
      ...
      {isDetailOpen && portalTarget && createPortal(
        <div className="fixed inset-0 z-[150] bg-slate-50 flex flex-col animate-in slide-in-from-right duration-300">
            <div className="px-6 pt-12 pb-6 flex items-center justify-between bg-white border-b border-slate-100">
                <button onClick={closeDetail} className="h-10 w-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-600 active:scale-95 transition-all">
                    <ChevronLeft size={20} />
                </button>
                ...
                <div className="p-8">
                    <div className="mb-8">
                        <div className="flex items-center gap-3 mb-2">
                             <h1 className="text-2xl font-bold tracking-tight text-slate-900" style={{ fontFamily: fonts.display }}>
                ...
                <div className="bg-white w-full max-w-sm rounded-[36px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                    <div className="p-8">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-xl font-bold text-slate-900" style={{ fontFamily: fonts.display }}>Loan Agreement</h3>
                        </div>
                        ...
                        <div className="space-y-4 mb-8">
                             ...
                             <div className="flex justify-between pb-3 border-b border-slate-50 text-sm">
                                <span className="text-slate-500 font-medium">Interest (10.5%)</span>
                                <span className="font-bold text-emerald-600">+{formatZar(interest)}</span>
                             </div>
                             <div className="bg-slate-900 rounded-3xl p-5 flex justify-between items-center shadow-lg shadow-slate-900/20">
            ...
            {workflowStep === "auth" && (
                <div className="bg-white w-full max-w-sm rounded-[36px] p-8 text-center shadow-2xl animate-in fade-in duration-300">
                    ...
                    <div className="flex justify-center gap-3 mb-10">
                        {[1,2,3,4].map(i => <div key={i} className="h-14 w-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-300 text-2xl font-bold">•</div>)}
                    </div>
                    <button 
                        onClick={handleConfirmPledge}
                        className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold uppercase tracking-widest text-xs shadow-lg flex items-center justify-center active:scale-95 transition-all"
                    >
                        {isProcessing ? <div className="h-5 w-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "Confirm Transaction"}
                    </button>
                </div>
            )}
