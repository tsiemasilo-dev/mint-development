...
export default function ChildDashboardPage({ child: initialChild, onBack }) {
  ...
  async function handleCompleteAddress() {
    ...
    try {
      ...
      console.error("[complete-address]", e);
    } finally {
          ...
          {isProfileIncomplete && (
            <motion.div variants={item}>
              <button
                ...
                disabled={completingAddress}
                className="w-full flex items-center gap-3 rounded-2xl px-4 py-4 text-left transition active:scale-[0.98] disabled:opacity-60"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-amber-800">
                    {completingAddress ? "Completing…" : `Complete ${child?.first_name}'s profile`}
                  </p>
                  <p className="text-xs text-amber-700 mt-0.5 truncate">
                    Missing: proof of address, responsibility agreement
                  </p>
                </div>
