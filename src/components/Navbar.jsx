...
const Navbar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <>
      {/* 1. Backdrop Blur (Excludes the Button via Z-index) */}
      {createPortal(
        <AnimatePresence>
          {isOpen && (
            <motion.div
      ...
      {/* 3. Bottom Navbar */}
      <nav
        className={`fixed bottom-0 left-0 right-0 z-[80] border-t border-white/20 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-2 transition-all duration-500 ${
              ...
              return (
                <div key={tab.id} className="flex flex-1 items-center justify-center">
                  <button
                    ...
                    className="relative z-[90] flex h-14 w-14 items-center justify-center rounded-full bg-slate-900 text-white shadow-xl transition-all active:scale-90"
                  >
