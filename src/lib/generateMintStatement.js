...
// ─── Logo cache & rendering (matches factsheet approach: atob + Blob URL) ─────
...
async function renderLogoToPng(h) {
  ...
  return new Promise((resolve, reject) => {
    const svgBlob = new Blob([atob(LOGO_SVG_B64)], { type: "image/svg+xml;charset=utf-8" });
    ...
    img.onload = () => {
      ...
      try { resolve(canvas.toDataURL("image/png", 0.94)); }
      catch (e) { reject(e); }
    };
...
async function drawLogo(doc, x, y, h) {
  if (h <= 0) return;
  if (!logoCache.has(h)) {
    try { logoCache.set(h, await renderLogoToPng(h)); }
    catch { _drawLogoFallback(doc, x, y, h); return; }
  }
  doc.addImage(logoCache.get(h), "PNG", x, y, h * LOGO_ASPECT, h);
}
...
// ─── Disclosure sections — matches factsheet exactly, with isDiamond styling ──
const DISC_SECTIONS = [
  {
...
async function addDisclosurePage(doc, clientName, accountNo, isoDate) {
  ...
  const LINE_H = 3.0, SEC_GAP = 5.5, PILL_H = 6.8, startY = HDR_H + 10;
  ...
  function renderCol(secs, colX, colW) {
    let y = startY;
    secs.forEach(sec => {
      // isDiamond sections use lighter header background (matches factsheet)
      const headerBg  = sec.isDiamond ? [232, 226, 252] : P;
      const headerTc  = sec.isDiamond ? P : WHITE;
      const dotCol    = sec.isDiamond ? P_MID : WHITE;

      fc(doc, headerBg); doc.roundedRect(colX, y, colW, PILL_H, 1.5, 1.5, "F");
      fc(doc, dotCol); doc.rect(colX + 3, y + 2.4, 1.6, 1.6, "F");
      doc.setFont("helvetica", "bold"); doc.setFontSize(6.4); tc(doc, headerTc);
      doc.text(sec.title.toUpperCase(), colX + 6.8, y + 4.5);
      y += PILL_H + 3.5;
      doc.setFont("helvetica", "normal"); doc.setFontSize(6.1); tc(doc, BODY);
  ...
  fc(doc, [240, 236, 255]); doc.roundedRect(ML, disclaimerY, CW, disclaimerH, 2.5, 2.5, "F");
  dc(doc, P_MID); doc.setLineWidth(0.6); doc.roundedRect(ML, disclaimerY, CW, disclaimerH, 2.5, 2.5, "S");
  doc.setFont("helvetica", "bold"); doc.setFontSize(7); tc(doc, P);
  doc.text("DISCLAIMER & LEGAL NOTICE", ML + 3, disclaimerY + 6);
  hl(doc, ML + 3, disclaimerY + 8.5, PW - MR - 3, [210, 200, 240], 0.25);
  doc.setFont("helvetica", "normal"); doc.setFontSize(5.9); tc(doc, BODY);
  doc.text(disclaimerLines, ML + 3, disclaimerY + 12);

  const addInfoY = disclaimerY + disclaimerH + 7;
  if (addInfoY + 22 < PH - 16) {
    fc(doc, P_LITE); doc.roundedRect(ML, addInfoY, CW, 22, 2.5, 2.5, "F");
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); tc(doc, P);
    doc.text("ADDITIONAL INFORMATION & CONTACT", ML + 3, addInfoY + 6);
    doc.setFont("helvetica", "normal"); doc.setFontSize(6.1); tc(doc, BODY);
    const addLines = doc.splitTextToSize(
      "Additional information regarding Mint's investment strategies — including strategy descriptions, risk disclosures, fee schedules, investment methodology, and portfolio construction framework — is available on request from Mint Platforms (Pty) Ltd.\ninfo@mymint.co.za  ·  +27 10 276 0531  ·  www.mymint.co.za  ·  3 Gwen Lane, Sandown, Sandton, Johannesburg.",
      CW - 6
    );
...
export const generateMintStatement = async (
  ...
  doc.setProperties({
    title: "MINT Investment Statement",
    subject: "MINT Investment Statement",
    author: "MINT Platforms (Pty) Ltd",
  });
  ...
  // Pre-warm logo cache
  try { if (!logoCache.has(9)) logoCache.set(9, await renderLogoToPng(9)); } catch {}
  ...
  if (!txForPdf.length) {
    ...
    autoTable(doc, {
      ...
      body: txForPdf.map(t => {
        let status = "—";
        if (t.status) {
          if (["successful","completed","posted"].includes(t.status)) status = "Completed";
  ...
  // ── SECTION 5: DISCLOSURES (dedicated page) ────────────────────────────────
  ...
  try {
    ...
    console.error("PDF output failed:", err);
    doc.save(filename);
  }
};
