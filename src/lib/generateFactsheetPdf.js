...
function drawSectorBars(doc, sectors, x, y, w) {
  ...
  sectors.forEach((s) => {
    ...
    const lines = doc.splitTextToSize(s.name, maxLabelW);
    const textStartY = cy + barH / 2 + 1.5 - ((lines.length - 1) * 2.5);
    lines.forEach((line, i) => {
      doc.text(line, x, textStartY + i * 2.5);
    });
