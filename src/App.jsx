...
const App = () => {
  ...
  useEffect(() => {
    const RIPPLE_CLASS = 'origin-ripple-el';
    const SIZE = 500;

    const getOrCreateRipple = (btn) => {
      let ripple = btn.querySelector(`.${RIPPLE_CLASS}`);
      if (!ripple) {
        ...
        if (!prev || prev === 'visible') btn.style.overflow = 'hidden';
    ...
    const onEnter = (e) => {
      ...
      const ripple = getOrCreateRipple(btn);
